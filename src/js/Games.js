...
var e = false,
    ...
    loginOrigin = "https://login." + game.backendHost,
    loginAuthType = 0,
    loginNonce = null;
...
var receiveLoginMessage = function(e) {
    ...
    if (e.data.authType !== loginAuthType) {
        console.log("authentication provider does not match: " + e.data.authType);
        return;
    }
    ...
    window.loginSuccess(e.data.session);
}
 ...
 *               authType: …,   // number indicating which authentication provider
 *               nonce:   "…",  // randomly generated by client, for CSRF prevention
 *               session: "…"   // token identifying user session
...
Games.popupLogin = function(e) {
    ...
    var url = loginOrigin + "/login?authtype=" + loginAuthType.toString() + "&nonce=" + loginNonce + "&origin=" + window.origin;
    openLoginWindow(url, "Login", 4 == e ? 900 : 500, 500)
};
...
Games.playerAuth = function() {
    Tools.ajaxPost("https://" + game.backendHost + "/auth", {
        session: config.settings.session
    }, function(e) {
        if (null != e) {
            game.loggedIn = true,
            game.myUserID = e.user;
            var t = UI.escapeHTML(e.authName.substr(0, 30)) + '<span class="grey">(' + ["", "Microsoft", "Google", "Twitter", "Reddit", "Twitch"][e.authType] + ")</span>",
                n = t + '<span class="link" onclick="Games.logout()">Logout</span>',
                r = "Logged in as " + t + '<span class="button" onclick="Games.logout()">LOG OUT</span>';
            null != e.flag && (game.myFlag = e.flag);
